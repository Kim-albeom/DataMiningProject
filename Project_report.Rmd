...
```
 ...
 각 모델에 대해서 정확도를 구하고 시각화를 했습니다. 위와 같이 $Decision\ Tree \lt Random\ Forest \lt Logistic\ Regression$ 순서로 정확도의 성능을 보입니다. 그러나 여기서 $Random\  Forest$ 와 $Logistic\  Regression$의 정확도가 비슷하게 나와 AUROC도 비교해보도록 하겠습니다.
...
```
 ...
 위 ROC Curve를 보았을 때 $Decision\ Tree \lt Random\ Forest \approx Logistic\ Regression$ 인 것을 확인할 수 있습니다. 또한 여기서도 Accuracy를 보았을 때와 동일하게 $Random\ Forest$와 $Logistic\ Regression$의 차이가 크지 않아 보이기 때문에 수치적으로 정확하게 확인해보겠습니다.
...
```
 ...
 각 모델에 대해서 AUROC를 구하고 시각화를 해줬습니다.$Random\ Forest$와 $Logistic\ Regression$의 차이가 크지는 않지만 $Decision\ Tree \lt Random\ Forest \lt Logistic\ Regression$의 순서로 AUROC 성능을 보이는 것을 확인할 수 있습니다.
 
<br><br> 

#### 3-1-3. Compare All

$Logistic\ Regression,\ Decision\ Tree,\ Random\ Forest$에 대해서 __Accuracy__와 __AUC__를 비교했습니다. 이를 동시에 비교해 보겠습니다.
...
> $Logistic\ Regression,\ Random\ Forest$ 두 모델이 상대적으로 $Decision\ Tree$에 비해서 성능이 좋게 나타납니다.
> $Logistic\ Regression$이 $Random\ Forest$에 비해서 미미하지만 더욱 좋은 성능을 보이고 있습니다.
> 따라서 모형의 안정성과 해석적인 부분을 고려했을 때, $Logistic\ Regression$을 이용하여 Scoring model을 개발하는 것이 바람직하다는 판단을 했습니다.

<br><br>

### 3-2. Enhance Logistic Regression Model
 $Logistic\ Regression$을 학습할 때, cutoff를 0.5로 지정하여 트리모델 간의 동일한 조건 하에 비교를 진행하였는데, $Logistic\ Regression$에 대해서 cutoff값을 다양하게 하여 비교를 했습니다.
...
print(results)
```
 위에서 ML 모형과의 비교를 할 때, cutoff값을 0.5로 설정했고, 이번에는 cutoff값에 변화를 주면서 차이를 확인했는데, 유의미한 변화가 보이지 않아서 기존에 했던 0.5로 cutoff값을 확정지어서 진행했습니다.
...
```
 ...
 __Accuracy__, __AUROC__ 값은 모델 간의 차이가 유의미하지 않아 __AIC__가 낮은 model 중 backward를 사용한 $Logistic\ Regression$ 모형을 이용해서 평점표 모형을 개발하겠습니다.
...
* 종속변수 
  - __diagnosis__: 진단 결과("B": 양성, "M": 악성)
* 독립변수
  - __radius_mean__: 중심에서 둘레까지의 거리의 평균
  - __texture_mean__: gray-scale 값의 평균
  - __smoothness_mean__: 반지름 길이의 지역 변동의 평균
  - __compactness_mean__: $radius^2/area-1.0$의 평균
  - __symmetry_mean__: 대칭성의 평균
  - __radius_se__: 중심에서 둘레까지의 거리의 표준오차
  - __texture_se__: gray-scale 값의 표준오차
  - __fractal_dimension_se__: $(costline\ approximation)^-1$의 표준오차
...
각 class 값에 계수값을 곱해주는 방법으로 구현했습니다.
...
<br>
 ...
 그 후 각 값에 2000을 더해주고 반올림해서 정수 값으로 표현했는데, 2000을 더해준 이유는 모든 값을 양의 정수로 표현하기 위해서 입니다.
...
#### 4-2-2. 2nd Scoring Model
 두 번째 평점표 모형은 각 변수의 계수값을 구간의 급간 값에 곱해주고, 그 후 다시 10을 곱한 후 일의 자리까지 반올림 해준 후, class가 증가 할때 1씩 증가하도록 했습니다.
...
> 예를 들어 radius_mean의 구간의 너비는 3입니다. 따라서 3에 계수 값인 1.0262을 곱한 후 10를 다시 곱해주고 일에 자리까지 반올림을 해줍니다.

> $round(3 * 1.0262 * 10)$을 해주는 겁니다. 그 값이 class 1의 점수가 되고, class 2, 3, 4는 1씩 더해준 값이 됩니다.

<br>

|Variable             | 1    | 2     | 3    | 4    |
|:--------------------|-----:|------:|-----:|-----:|
|radius_mean          |  31  | 32    | 33   | 34   |
|texture_mean         |  13  | 14    | 15   | 16   |
|smoothness_mean      |  8   | 9     | 10   | 11   |
|compactness_mean     |  4   | 5     | 6    | 7    |
|symmetry_mean        |  84  | 85    | 86   | 87   |
|radius_se            |  5   | 6     | 7    | 8    | 
|texture_se           | -4   | -5    | -6   | -7   |
|fractal_dimension_se | -4   | -5    | -6   | -7   |
...
#### 4-2-3. 3rd Scoring Model
 세 번째 평점표 모형은 두 번째 모형과 동일하게 계수와 구간의 너비를 곱하고, 10을 곱한 후 일의 자리까지 반올림한 것은 동일하지만, class가 증가할 때, 구간의 너비만큼 값을 더해주었습니다. 이는 2번째 모형보다 score의 분포를 퍼지게 하고 변수에 대한 가중치가 적절히 반영되도록 해주기 위함입니다.
 
<br>

> 2nd model: ith class = round(계수 * 구간너비 * 10) + 1 * (i-1)

> 3rd model: ith class = round(계수 * 구간너비 * 10) + 3 * (i-1)

<br>

|Variable             | 1    | 2     | 3    | 4    |
|:--------------------|-----:|------:|-----:|-----:|
|radius_mean          |  31  | 34    | 37   | 40   |
|texture_mean         |  13  | 16    | 19   | 22   |
|smoothness_mean      |  8   | 9     | 10   | 11   |
|compactness_mean     |  4   | 8     | 8    | 10   |
|symmetry_mean        |  84  | 87    | 90   | 93   |
|radius_se            |  5   | 6     | 7    | 8    | 
|texture_se           | -4   | -5    | -6   | -7   |
|fractal_dimension_se | -4   | -5    | -6   | -7   |
...
> 3개의 스코어링 모델 중 먼저 2, 3번 모델의 AUROC, KS 통계량이 좋은 성능을 보이고 있고, 2번 모델의 경우 3번 모델보다 AUROC, KS 통계량이 더 좋습니다.

> 그러나 일반적으로 KS 통계량이 너무 높으면 의심할 필요가 있다고 합니다. 2번 모델의 KS 통계량이 0.826251로 선택하기 안정적이지 않다고 판단했습니다.

> 3번 모델은 KS 통계량이 0.751532이고, 누적분포를 2번과 3번 모델을 비교했을 때, 3번 모델이 더 score의 분포가 더 넓게 포진되어 있어서 조금 더 안정적이라 판단했습니다. 따라서 3번 모델을 스코어링 모델로 사용하기로 결정했습니다.
